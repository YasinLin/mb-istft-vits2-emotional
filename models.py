...
class TextEncoder(nn.Module):
    def __init__(self,
        ...
        # self.emb_bert = nn.Conv1d(256, hidden_channels, 1)
    ...
    def forward(self, x, x_lengths, emo, bertpt, g=None):
        # print(self.emb(x).shape)
        x = self.emb(x) * math.sqrt(self.hidden_channels)  # [b, t, h]
        b = self.emb_bert(bertpt)
        e = self.emo_proj(emo.unsqueeze(1))
        # print(x.shape, b.shape, e.shape, bert.shape, emo.shape)
        x = x + b + e
...
class SynthesizerTrn(nn.Module):
  ...
  """
    ...
    def forward(self, x, x_lengths, y, y_lengths, sid=None, emo=None, bertpt=None):
        ...
        x, m_p, logs_p, x_mask = self.enc_p(x, x_lengths, emo, bertpt, g=g)  # vits2?
    ...
    def infer(self, x, x_lengths, sid=None, emo=None, bertpt=None, noise_scale=1, length_scale=1, noise_scale_w=1., max_len=None):
        if self.n_speakers > 0:
            g = self.emb_g(sid).unsqueeze(-1)  # [b, h, 1]
        else:
            g = None
        x, m_p, logs_p, x_mask = self.enc_p(x, x_lengths, emo, bertpt, g=g)
        if self.use_sdp:
